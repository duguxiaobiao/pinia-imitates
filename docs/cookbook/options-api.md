...
* [mapState](./../core-concepts/state.html#usage-with-the-options-api)
* [mapWritableState](./../core-concepts/state.html#modifiable-state)
* ⚠️ [mapGetters](./../core-concepts/getters.html#without-setup) (只是为了迁移方便，请用 `mapState()` 代替)
* [mapActions](./../core-concepts/actions.html#without-setup)
...
默认情况下，所有映射辅助函数都支持自动补全，你不需要做任何事情。如果你调用 `setMapStoreSuffix()` 修改了 `"Store"` 的后缀，你还需要在 TS 文件或 `global.d.ts` 文件的某个地方添加它。
最方便的地方就是你调用 `setMapStoreSuffix()` 的地方：
