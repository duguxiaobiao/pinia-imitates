...
<VideoComponent href="https://masteringpinia.com/lessons/What-is-a-pinia-plugin"/>
...
> [!WARNING]
> 如果你使用的是 Vue 2，Pinia 与 Vue 一样，受限于[相同的响应式限制](https://v2.cn.vuejs.org/v2/guide/reactivity.html#%E6%A3%80%E6%B5%8B%E5%8F%98%E5%8C%96%E7%9A%84%E6%B3%A8%E6%84%8F%E4%BA%8B%E9%A1%B9)。
> 在创建新的 state 属性时,如 `secret` 和 `hasError`，你需要使用 `Vue.set()` (Vue 2.7) 或者 `@vue/composition-api` 的 `set()` (Vue < 2.7)。
> ```js
...
你也可以在插件中使用 [store.$subscribe](./state.html#subscribing-to-the-state) 和 [store.$onAction](./actions.html#subscribing-to-actions) 。
...
当[在 Nuxt 中使用 pinia](./../ssr/nuxt.html) 时，你必须先创建一个 [Nuxt 插件](https://nuxt.com/docs/guide/directory-structure/plugins)。这样你才能访问到 `pinia` 实例：
